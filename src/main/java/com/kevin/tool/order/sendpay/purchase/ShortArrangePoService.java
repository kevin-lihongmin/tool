package com.kevin.tool.order.sendpay.purchase;

import com.kevin.tool.order.sendpay.CodeApplicationContextImpl;
import com.kevin.tool.order.sendpay.Stage;
import com.kevin.tool.order.sendpay.generate.param.PurchaseCodeParam;
 ...
 */
@Service
public class ShortArrangePoService {

    private CodeApplicationContextImpl context;

    @Autowired
    public ShortArrangePoService(CodeApplicationContextImpl context) {
    ...
    public void turnOrder() {
        // 销售开单检查服务
        Boolean check = context.check(new PurchaseCodeParam(), Stage.SALE_CREATE);
        if (!check) {
            // 不通过则设置状态
            saveState();
            return;
