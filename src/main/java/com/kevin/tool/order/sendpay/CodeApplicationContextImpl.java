...
import com.kevin.tool.order.sendpay.check.CheckCodeContext;
import com.kevin.tool.order.sendpay.check.CheckService;
import com.kevin.tool.order.sendpay.check.PreposingStateConfig;
import com.kevin.tool.order.sendpay.check.marker.DefaultMarkerFlagService;
import com.kevin.tool.order.sendpay.check.marker.MarkerCheckService;
import com.kevin.tool.order.sendpay.check.marker.MarkerFlagService;
import com.kevin.tool.order.sendpay.generate.CodeFactory;
 ...
 * @see #check(CodeParam, Stage) 每个节点是否检查通过
 *
...
public class CodeApplicationContextImpl extends CheckCodeContext implements MarkerCheckService, CodeFactory {
    ...
    private static Map<PreposingStage, List<CheckService>> CONFIG_SERVICE_MAP;
    ...
    public void afterPropertiesSet() {
        ...
        PreposingStage[] states = PreposingStage.values();
        Map<PreposingStage, List<CheckService>> configMap = new HashMap<>(2);

        for (PreposingStage preposingStage : states) {
            PreposingStateConfig status = PreposingStateConfig.getStatus(preposingStage);
            ...
            if (!status.getCheckList().isEmpty()) {
                ...
                configMap.put(preposingStage, checkServices);
            }
        }
    ...
    public CheckDTO chaseCheckAndFlag(CodeParam codeParam, PreposingStage preposingStage) {
        ...
        Boolean aBoolean = invokeService(booleans, preposingStage);
    ...
    @Override
    public CheckDTO saleCheckAndFlag(CodeParam codeParam, PreposingStage preposingStage) {
        // 查询销售订单配置服务
        Boolean[] booleans = new Boolean[0];
        Boolean aBoolean = invokeService(booleans, preposingStage);
        return CheckDTO
     ...
     * @return 是否检查通过
     */
    private Boolean invokeService(Boolean[] checkList, PreposingStage preposingStage) {
        int i = 0;
        List<CheckService> checkServices = CONFIG_SERVICE_MAP.get(preposingStage);
